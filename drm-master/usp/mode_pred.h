...
typedef struct _mode_pred_info {
  ...
  // total number of nonzero transform coefficients for each luma transform block
  int *TotalCoeffL;
  // stores the Intra_4x4 prediction mode of each luma block if it is encoded using Intra4x4 prediciotn (-1 ow)
  int *Intra4x4PredMode;
  // motion vectors for blocks that are encoded using inter prediciton (MV_NA ow)
  int *MVx,*MVy;
} mode_pred_info;
