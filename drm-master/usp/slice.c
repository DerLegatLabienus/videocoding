...
void decode_slice_data(slice_header *sh,
  ...
  int* LumaACLevel   = malloc( 16*16*sizeof(int));
  int* ChromaDCLevel = malloc(   2*4*sizeof(int));
  int* ChromaACLevel = malloc(2*4*16*sizeof(int));

#define LAO(a,b)   (  16*(a) +    (b)      )
#define CDO(a,b)   (   4*(a) +    (b)      )
#define CAO(a,b,c) (16*4*(a) + 16*(b) + (c))
...
//printf("\n[%d:%d] %d,%d ",frame_no,CurrMbAddr,mb_pos_x,mb_pos_y);_dump_mb_mode(&mb);
      ...
      if(mb.mb_type==I_PCM) {  // I_PCM macroblock ////////////////////////////
        ...
        for(y=0; y<2; ++y)
          ...
          if (debug) fprintf(stderr, "\"normal\" macroblock\n");
        if(mb.MbPartPredMode[0]!=Intra_4x4 &&
           mb.MbPartPredMode[0]!=Intra_16x16 &&
           mb.NumMbPart==4)
        { // sub_mb_pred() ////////////////////////////////////////////////////
            if (debug) fprintf(stderr, "sub_mb_pred()\n");
...
//{int i;printf("sub_mb_pred():");for(i=0;i<4;++i)printf(" %s(%d)",_str_sub_mb_type(sub[i].sub_mb_type),sub[i].NumSubMbPart);printf("\n");}
          ...
          for(mbPartIdx=0; mbPartIdx<4; ++mbPartIdx)
            ...
            if (debug) fprintf(stderr, "mb_pred(), mb_pos_x=%d, mb_pos_y=%d\n", mb_pos_x, mb_pos_y);
            if (debug) {
                int i,j;
                fprintf(stderr, "nal_pos = %d\n", nal_pos);
                for (i=0; i < 4; ++i) {
                    for (j=0; j < 8; ++j) {
                        fprintf(stderr, "%2x|", *(nal_buf + nal_pos + i*8+j));
                    }
                    fprintf(stderr, "\n");
                }
            }
...
//printf("intra_4x4:");
              for(luma4x4BlkIdx=0; luma4x4BlkIdx<16; ++luma4x4BlkIdx) {
                ...
                if (debug) fprintf(stderr, "predIntra4x4PredMode=%d\n", predIntra4x4PredMode);
                if(input_get_one_bit()) { // prev_intra4x4_pred_mode_flag
                    if (debug) fprintf(stderr, "prev_intra4x4_pred_mode_flag set\n");
                ...
                //#define ModePredInfo_Intra4x4PredMode(mpi,x,y) (mpi->Intra4x4PredMode[(y)*mpi->TbPitch+(x)])
                  ...
                  int rem_intra4x4_pred_mode=input_get_bits(3);
                    if (debug) fprintf(stderr, "prev_intra4x4_pred_mode_flag not set, rem = %d\n", rem_intra4x4_pred_mode);
...
//printf("\n");
            ...
            if (debug) fprintf(stderr, "intra_chroma_pred_mode = %d\n", intra_chroma_pred_mode);
/* fprintf(stderr, " intra_chroma_pred_mode: %d\n",intra_chroma_pred_mode); */
          } else { // mb_pred() for inter macroblocks /////////////////////////
            if (debug) { fprintf(stderr, "mb_pred() for inter macroblock\n"); }
            int mbPartIdx;
            // ignoring ref_idx_* and *_l1 stuff for now -- I do not support
            // long-term prediction or B-frames anyway ...
            ...
            for(mbPartIdx=0; mbPartIdx<mb.NumMbPart; ++mbPartIdx)
              if(mb.MbPartPredMode[mbPartIdx]!=Pred_L1) {
                int mvdx=get_signed_exp_golomb();
                int mvdy=get_signed_exp_golomb();
                if (debug) fprintf(stderr, "mvdx=%d, mvdy=%d\n", mvdx, mvdy);
                //mvdx *= -1;
                //printf("dump mv2: %d:%d\n", mvdx, mvdy);
                DeriveMVs(mpi,
...
//printf("coded_block_pattern=%d\n",coded_block_pattern);
            if (debug) fprintf(stderr, "coded_block_pattern = %d\n", coded_block_pattern);
...
//_dump_mb_mode(&mb);
        ...
        if (debug) fprintf(stderr, "memset coefficients to zero\n");
        memset(LumaDCLevel,0,sizeof(LumaDCLevel));
        memset(LumaACLevel,0,sizeof(int)*16*16);
        memset(ChromaDCLevel,0,sizeof(int)*2*4);
        memset(ChromaACLevel,0,sizeof(int)*2*4*16);
...
*/
        ...
        if(mb.MbPartPredMode[0]==Intra_4x4) {  ///////////////// Intra_4x4_Pred
          ...
          for(i=0; i<16; ++i) {
            ...
            /*     int j; */
            /*     for (j=0; j<16; ++j) */
            /*         fprintf(stderr, "%d, ", LumaACLevel[LAO(i,j)]); */
            /*     fprintf(stderr, "\n"); */
            /* } */
          }
