...
#define GET_HALF(i) ((i) <= 8 ? 0 : 1)
  ...
  for (i=0; i < MIN(i_total, 16); ++i) {
      p[GET_HALF(i)] |= (runlevel.level[i] > 0) << i;
  }
  rc4_crypt(&h->rc4, p, c, GET_HALF(i_total) + 1);

  // modify the levels such that their signs will now match c
  for (i=0; i < MIN(i_total, 16); ++i) {
