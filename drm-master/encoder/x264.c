...
static struct option long_options[] =
{
    ...
    { "rc4-keyfile",     required_argument, NULL, OTP_RC4_KEYFILE },
    {0, 0, 0, 0}
};
...
static int parse( int argc, char **argv, x264_param_t *param, cli_opt_t *opt )
{
    ...
    memset(param->rc4_keyfile, 0, 256);
    ...
    for( optind = 0;; )
    {
        ...
        switch( c )
        {
            ...
            case OPT_INPUT_RANGE:
                ...
                break;
            case OTP_RC4_KEYFILE:
                arglen = strlen(optarg);
                if (arglen > 256) arglen = 256;
                memset(param->rc4_keyfile, 0, 256);
                memcpy(param->rc4_keyfile, optarg, arglen);
                break;
            case OPT_RANGE:
...
#   define  MAX_PTS_WARNING 3 /* arbitrary */
    ...
    unsigned char rc4key[256] = {0};
    //rc4_init(&h->rc4, rc4key, 256);
    if (*(param->rc4_keyfile)) {
        x264_cli_log("rc4", X264_LOG_INFO, "param->rc4_keyfile %s\n", param->rc4_keyfile);
        FILE* keyfile = fopen(param->rc4_keyfile, "rb");
        int res = 0;
        if (keyfile) {
            res = fread(rc4key, 1, 256, keyfile);
            printf("\n");
            for (int i = 0; i != 16; ++i) {
                for (int j = 0; j != 16; ++j) {
                    printf("%02x |", rc4key[i * 16 + j]);
                }
                printf("\n");
            }
        ...
        FAIL_IF_ERROR2( res != 256, "could not read rc4 key, res = %d\n", res );
    }
