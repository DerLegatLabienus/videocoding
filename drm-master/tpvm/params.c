...
int check_unsupported_features(seq_parameter_set *sps, pic_parameter_set *pps) {
  ...
  #define UNSUPPORTED(feature) \
    do { \
      printk(KERN_WARNING "UNSUPPORTED FEATURE: %s\n",feature); \
      ++unsupported_count; \
    } while(0)

  #define UNSUPPORTED2(feature, param) \
    do { \
      printk(KERN_WARNING "UNSUPPORTED FEATURE: %s - %d\n",feature, param); \
      ++unsupported_count; \
    } while(0)

  //if(sps->pic_order_cnt_type!=0) 
    //UNSUPPORTED2("picture order count type != 0 : ", sps->pic_order_cnt_type);
  ...
  if(sps->frame_cropping_flag)
    printk(KERN_WARNING "UNSUPPORTED FEATURE: frame cropping (ignored)\n");
