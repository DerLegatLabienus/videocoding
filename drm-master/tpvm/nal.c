...
#define gnn_add_segment(end) do { \
    unsigned char *dst = &nal_buf[nalu_size], *src = &ring_buf[segment_start]; \
    while (src != &ring_buf[end]) { \
        /* skip nal escaping */ \
        if (3 == src[0] && !src[-1] && !src[-2]) \
            src++; \
        else { \
            *dst++ = *src++; \
            nalu_size+=1; \
...
int get_next_nal_unit(nal_unit *nalu) {
    ...
    for(;;) {
        if(input_remain<=3) return 0;
        if((!ring_buf[ring_pos]) &&
                (!ring_buf[(ring_pos+1)&RING_MOD]) &&
                ( ring_buf[(ring_pos+2)&RING_MOD]==1))
            break;
        gnn_advance();
    }
    for(i=0; i<3; ++i) gnn_advance();
