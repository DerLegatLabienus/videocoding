...
mode_pred_info *alloc_mode_pred_info(int width, int height) {
  mode_pred_info *mpi=kmalloc(sizeof(mode_pred_info), FLAGS);
  int x,y;
  // per-macroblock information     (16x16)
  x=mpi->MbWidth=mpi->MbPitch=width>>4;
  y=mpi->MbHeight=height>>4;
  mpi->MbMode=kmalloc(x*y*sizeof(int), FLAGS);
  // per-chroma block information    (8x8)
  x=mpi->CbWidth=mpi->CbPitch=width>>3;
  y=mpi->CbHeight=height>>3;
  mpi->TotalCoeffC[0]=kmalloc(x*y*sizeof(int), FLAGS);
  mpi->TotalCoeffC[1]=kmalloc(x*y*sizeof(int), FLAGS);
  // per-transform block information (4x4)
  x=mpi->TbWidth=mpi->TbPitch=width>>2;
  y=mpi->TbHeight=height>>2;
  mpi->TotalCoeffL=kmalloc(x*y*sizeof(int), FLAGS);
  mpi->Intra4x4PredMode=kmalloc(x*y*sizeof(int), FLAGS);
  mpi->MVx=kmalloc(x*y*sizeof(int), FLAGS);
  mpi->MVy=kmalloc(x*y*sizeof(int), FLAGS);
  return mpi;
}
...
void clear_mode_pred_info(mode_pred_info *mpi) {
  ...
  if(mpi->Intra4x4PredMode) memset(mpi->Intra4x4PredMode,0xBB,mpi->TbPitch*mpi->TbHeight*sizeof(int));
...
void free_mode_pred_info(mode_pred_info *mpi) {
  if(!mpi) return;
  if(mpi->MbMode)           kfree(mpi->MbMode);
  if(mpi->TotalCoeffC[0])   kfree(mpi->TotalCoeffC[0]);
  if(mpi->TotalCoeffC[1])   kfree(mpi->TotalCoeffC[1]);
  if(mpi->TotalCoeffL)      kfree(mpi->TotalCoeffL);
  if(mpi->Intra4x4PredMode) kfree(mpi->Intra4x4PredMode);
  if(mpi->MVx)              kfree(mpi->MVx);
  if(mpi->MVy)              kfree(mpi->MVy);
  kfree(mpi);
}
