...
static void skip_ref_pic_list_reordering(void) {
  int reordering_of_pic_nums_idc;
  int abs_diff_pic_num;
  int long_term_pic_num;
  printk(KERN_WARNING "Warning: I do not support reference picture list reordering. Watch out for decoding errors!\n");
  do {
...
static void skip_adaptive_ref_pic_marking(void) {
  int memory_management_control_operation;
  int difference_of_pic_nums;
  int long_term_pic_num;
  int long_term_frame_idx;
  int max_long_term_frame_idx;
  printk(KERN_ERR "Warning: I do not support adaptive reference picture marking. Watch out for decoding errors!\n");
  do {
...
#define SI_SLICE  4
*/
//int golomb_to_slice_type[5] = {2, 3, 1, 6, 5};
...
void decode_slice_header(slice_header *sh,
  ...
  if((pps->weighted_pred_flag && (sh->slice_type==P_SLICE || sh->slice_type==SP_SLICE)) ||
    ...
    printk(KERN_ERR "sorry, I _really_ do not support weighted prediction!\n");
    //exit(1); TODO: any kernel equivalent?
  }
